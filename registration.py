...
#checking if the data has filled the all feild or not.
    ...
    elif user.get() == '' or address.get() == '' or email.get() == '' or contact.get() == '' or password.get() == '' or con_password.get() == '' or payment_method == '' :
         messagebox.showerror('Error', 'All field must be filled.')
        ...
        if phone.isalnum():
            ...
            if len(val) > 10 or len(val) < 10:
                ...
                if psw.isalnum() and psw != 'Type password':
                    ...
                    elif payment_method.get() == '':
                        messagebox.showinfo('Payment method ', 'please select the payment method')

                    else:
                        username = user.get()
                        record = search(username)
                        if record != None:
                            ...
                            values = ['', user.get(), address.get(), email.get(), contact.get(), password.get(), payment_method.get()]
